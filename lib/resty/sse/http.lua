...
-- SSE specific method added to lua-resty-http library
function _M.w_body_reader(self, sock, content_length, default_chunk_size)
    return _body_reader(sock, content_length, default_chunk_size)
end
...
function _M.read_response(self, params)
    ...
    if _should_receive_body(params.method, status) then
        ...
        if ok and version == 1.1 and (encoding == "chunked" or string.find(res_headers["Content-Type"], "text/event-stream") ~= nil) then
